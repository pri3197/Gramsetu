...
LoggerFactory.getLogger(FisheriesController.class);
...
log.error("Could not fetch fish map data from Python service: {}",
e.getMessage());
}
...
log.error("Could not fetch reproduction bans from Python service: {}",
e.getMessage());
}
return ResponseEntity.ok(new ArrayList<>());
}
...
log.
info("Fetching historical fish census population data from Python service");
try {
...
log.error("Could not fetch historical trends from Python service: {}",
e.getMessage());
}
return ResponseEntity.ok(new ArrayList<>());
}
...
log.error("Could not fetch fisheries schemes from Python service: {}",
e.getMessage());
}
return ResponseEntity.ok(new ArrayList<>());
}
...
log.info("Received marine mammal sighting: {} at [{}, {}]", species,
latitude, longitude);
...
log.
info("Fetching live fishermen warnings from Python service: {}/fisheries/imd-warnings"
, pythonServiceUrl);
try {
...
log.error("Could not fetch IMD warnings from Python service: {}",
e.getMessage());
}
return
ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).body(Map.of("error",
"IMD service unreachable"));
}

@GetMapping("/mangroves/historical")
public ResponseEntity<?> getHistoricalMangroves() {
log.
info("Fetching FSI historical mangrove cover from Python service: {}/fisheries/mangroves/historical"
, pythonServiceUrl);
try {
...
log.error("Could not fetch historical mangrove data from Python service: {}",
e.getMessage());
}
return
ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE).body(Map.of("error",
"Mangrove service unreachable"));
}
}
