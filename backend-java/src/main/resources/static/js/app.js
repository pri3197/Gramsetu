...
// 12. Mangrove Canopy Simulation & Live IMD Fisherman Warnings Logic
let mangroveData = [];
let activeMangroveYear = 2023;
let mangroveCircles = [];
let selectedMangroveState = null;
let imdWarnings = [];

async function loadMangroveDataAndWarnings() {
    try {
        // Fetch real FSI mangrove cover data
        const resMangroves = await fetch('/api/fisheries/mangroves/historical');
        if (resMangroves.ok) {
            mangroveData = await resMangroves.json();
        }
        
        // Fetch dynamic IMD warnings
        const resWarnings = await fetch('/api/fisheries/imd-warnings');
        if (resWarnings.ok) {
            imdWarnings = await resWarnings.json();
        }
        
        renderMangroveCircles();
        renderImdWarnings();
        updateSimulationUI();
    } catch (e) {
        console.error("Error loading mangrove simulation or warnings: ", e);
    }
}

function renderMangroveCircles() {
    if (!fisheriesMap || !mangroveData.length) return;
    
    // Clear old circles
    mangroveCircles.forEach(c => fisheriesMap.removeLayer(c));
    mangroveCircles = [];
    
    const yearData = mangroveData.filter(d => d.year === activeMangroveYear);
    const yearsList = [2005, 2009, 2011, 2013, 2015, 2017, 2019, 2021, 2023];
    const yearIdx = yearsList.indexOf(activeMangroveYear);
    
    yearData.forEach(d => {
        // Find previous cover value to determine increase/decrease
        let coverChange = 0;
        let color = '#fbbf24'; // Orange default
        
        if (yearIdx > 0) {
            const prevYear = yearsList[yearIdx - 1];
            const prevRec = mangroveData.find(x => x.state === d.state && x.year === prevYear);
            if (prevRec) {
                coverChange = d.cover - prevRec.cover;
                if (coverChange > 0) {
                    color = '#10b981'; // Green for growth
                } else if (coverChange < 0) {
                    color = '#ef4444'; // Red for deforestation
                }
            }
        } else {
            color = '#10b981'; // Default green for initial year (2005 baseline)
        }
        
        // Size proportional to cover (using sqrt for visual area scaling)
        const radius = Math.sqrt(d.cover) * 12000;
        ...
        if (yearIdx > 0) {
            changeText = coverChange >= 0 ? `+${coverChange} km²` : `${coverChange} km²`;
        }
        
        const popupContent = `
            <div style="font-family: var(--font-body); padding: 5px;">
                <h4 style="margin-bottom: 2px; color:var(--text-primary); font-weight:700;">${d.state}</h4>
                <p style="margin: 0; color:var(--text-secondary); font-size:0.85rem;">Year: <strong>${d.year}</strong></p>
                <p style="margin: 0; color:var(--text-secondary); font-size:0.85rem;">Mangrove Area: <strong style="color:var(--color-accent-light);">${d.cover} km²</strong></p>
                <p style="margin: 0; color:var(--text-secondary); font-size:0.85rem;">Temporal Change: <strong style="color:${color};">${changeText}</strong></p>
            </div>
        `;
        
        circle.bindPopup(popupContent);
        
        // Click action to display state stats in the side panel
        circle.on('click', () => {
            selectedMangroveState = d.state;
            updateSimulationUI();
        });
        
        mangroveCircles.push(circle);
    });
}

function updateMangroveYear(year) {
    activeMangroveYear = parseInt(year);
    document.getElementById('mangrove-slider-val').innerText = activeMangroveYear;
    
    // Update map overlays
    renderMangroveCircles();
    
    // Update dynamic satellite comparison imagery
    const imgEl = document.getElementById('mangrove-satellite-img');
    const badgeEl = document.getElementById('mangrove-satellite-year');
    if (imgEl && badgeEl) {
        if (activeMangroveYear <= 2013) {
            imgEl.src = "/uploads/mangrove_2010.png";
            badgeEl.innerText = "2010 Deforested";
            badgeEl.style.backgroundColor = "#ef4444";
        } else {
            imgEl.src = "/uploads/mangrove_2026.png";
            badgeEl.innerText = "2026 Restored";
            badgeEl.style.backgroundColor = "var(--color-accent)";
        }
    }
...
function updateSimulationUI() {
    const stateEl = document.getElementById('mangrove-selected-state');
    const valEl = document.getElementById('mangrove-canopy-val');
    
    if (!stateEl || !valEl) return;
    
    if (selectedMangroveState) {
        stateEl.innerText = selectedMangroveState;
        const rec = mangroveData.find(d => d.state === selectedMangroveState && d.year === activeMangroveYear);
        valEl.innerText = rec ? `${rec.cover.toFixed(1)} km²` : "N/A";
    } else {
        // Calculate National Total
        stateEl.innerText = "All India";
        const yearRecs = mangroveData.filter(d => d.year === activeMangroveYear);
        const total = yearRecs.reduce((sum, r) => sum + r.cover, 0);
        valEl.innerText = total > 0 ? `${total.toLocaleString()} km²` : "4,992.0 km²";
    }
}

function renderImdWarnings() {
    const listEl = document.getElementById('imd-warnings-list');
    if (!listEl) return;
    
    if (!imdWarnings || !imdWarnings.length) {
        listEl.innerHTML = `
            <div style="text-align: center; color: var(--text-secondary); padding: 2rem 0;">
                <i class="fa-solid fa-circle-check" style="font-size: 1.5rem; margin-bottom: 0.5rem; color: #10b981;"></i>
                <p style="font-size:0.85rem;">No active weather warnings. Sea conditions normal.</p>
            </div>
        `;
        return;
    }
    ...
    imdWarnings.forEach(w => {
        // Map icon or dynamic download link
        const fileLink = w.pdf_url ? w.pdf_url : "#";
        const warningImage = w.img_url ? `<img src="${w.img_url}" style="width:100%; border-radius:4px; margin-top:0.5rem; max-height:120px; object-fit:cover;" alt="IMD graphic">` : "";
        
        html += `
            <div class="warning-item-card" style="margin-bottom:0.75rem;">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:0.5rem;">
                    <div class="warning-zone-title">${w.zone}</div>
                    <span class="warning-badge-urgent">Advisory</span>
                </div>
                ${warningImage}
                <div style="display:flex; justify-content:flex-end; margin-top:0.5rem;">
                    <a href="${fileLink}" target="_blank" class="warning-pdf-btn">
                        <i class="fa-solid fa-file-pdf"></i> Download Official Bulletin
                    </a>
                </div>
            </div>
        `;
